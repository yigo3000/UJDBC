        ...
        switch (typeHandle.getJdbcType()) {
            ...
            case Types.NUMERIC:
                ...
                if (scale < 0 || scale > precision) {
                    System.out.println("Scale "+scale+" precision:"+precision+"not support.");
                    System.out.println("getJdbcTypeName "+typeHandle.getJdbcTypeName()+", getColumnSize "+typeHandle.getColumnSize()+ ", getJdbcType "+typeHandle.getJdbcType());
                    if (numberDefaultScale > precision) {
                        precision = Decimals.MAX_PRECISION;
                        scale = Math.max(scale, 0);
                        System.out.println("Convert to scale: " +scale + "precision: " +precision );
                        return Optional.of(decimalReadMapping(createDecimalType(Decimals.MAX_PRECISION, scale)));
                    }
                    else {
                        System.out.println("Convert to numberDefaultScale: " +numberDefaultScale + "precision: " +precision );
